...
# Functions to create necessary tkinter widgets
def createwidgets():
    root.feedLabel = Label(root, bg="steelblue", fg="white", text="Webcam", font=('Arial',20))
    ...
    root.captureBTN = Button(root, fg="white", text="Capture", command=Capture, bg="steelblue", font=('Arial',20), width=20)
    root.captureBTN.grid(row=4, column=1, padx=10, pady=10)

    root.camBTN = Button(root, fg="white", text="Stop camera", command=StopCAM, bg="steelblue", font=('Arial',15), width=13)
    root.camBTN.grid(row=4, column=2)

    root.previewlabel = Label(root, bg="steelblue", fg="white", text="Image Preview", font=('Arial',20))
...
def Capture():
    ...
    # Displaying date and time on the frame (if we want to put a date/time on the picture, but it would not be the best for OCR)
...
def StopCAM():
    ...
    root.cameraLabel.config(text="Cam off", font=('Arial',70))
...
# Title, window size, background color and resizing property (disabled)
root.title("Pycam (Test)")
